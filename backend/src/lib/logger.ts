                    ...
                    : winston.format((logData) => {
                          ...
                          const resultLogData = {
                              ...
                              [MESSAGE]:
                                  [
                                      topMessage,
                                      Object.keys(visibleMessageTags).length > 0 ? `${EOL}${stringifyedLogData}` : '',
                                  ]
                                      .filter(Boolean)
                                      .join('') + EOL,
                          }
...
export const logger = {
    info: (logType: string, message: string, meta?: LoggerMetaData) => {
        if (!debug.enabled(`wordcraft:${logType}`)) {
            return
        }
    ...
    error: (logType: string, error: any, meta?: LoggerMetaData) => {
        ...
        const isExpectedError = error.cause instanceof ExpectedError
        const prettifiedMetaData = prettifyMeta(meta)
        if (!isNativeExpectedError && !isExpectedError) {
